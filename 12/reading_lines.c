...
//
// It's up to the direct caller to free() this pointer when done with it.
//
...
char *readline(FILE *fp)
{
    ...
    {                                             // if we're short of the end
        ...
        // otherwise we'll just leave buf as it is
        if (new_buf != NULL)
            buf = new_buf;
    }
...
int main(void)
{
    FILE *fp = fopen("/home/wangonya/code/bgc/12/malloc.c", "r");
